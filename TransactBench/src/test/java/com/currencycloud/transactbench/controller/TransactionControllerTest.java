...
class TransactionControllerTest {
    ...
    @BeforeEach
    void setUp() {
        request = new TransactionGenerationRequest("provider1", "sepa", 2);
    ...
    void generateTransactions_ShouldReturnCreatedWithTransactionIds_WhenSuccessful() {
        // Given
        when(generatorService.generateTransactions(anyString(), anyString(), anyInt()))
        ...
        verify(generatorService).generateTransactions("provider1", "sepa", 2);
    ...
    void generateTransactions_ShouldHandleLargeNumberOfTransactions() {
        ...
        TransactionGenerationRequest largeRequest =
                new TransactionGenerationRequest("provider1", "sepa", 1000);
        when(generatorService.generateTransactions(anyString(), anyString(), anyInt()))
                .thenReturn(mockTransactions);
        doNothing().when(publisherService).publishTransactions(any());
        ...
        verify(generatorService).generateTransactions("provider1", "sepa", 1000);
        verify(publisherService).publishTransactions(mockTransactions);
    }
